...
func (cs *CronService) VoidTrx(params transactiondata.TransactionData) {
	...
	data, err := cs.transactionService.GetDataByTrxID(context.Background(), req.TransactionID)
	if err != nil {
		cs.ErrorClear(context.Background(), params.ID, 0, 0)
		h.ErrorLog("Cron - Get data sale : " + err.Error())
		fmt.Println(err)
	}
	...
	id, err := cs.transactionService.CreateVoidTrx(context.Background(), trxParams)
	if err != nil {
		cs.ErrorClear(context.Background(), params.ID, 0, 0)
		h.ErrorLog("Cron - Save trx void : " + err.Error())
		fmt.Println(err)
	}
	...
	if params.DataResponse != "" {
		...
		if params.DataResponse != "E6" {
			...
			if err != nil {
				...
				h.ErrorLog("Cron - Unmarshal response void : " + err.Error())
				fmt.Println(err)
			}
		...
		err = cs.transactionService.UpdateVoidTrx(context.Background(), transactions.UpdateVoidParams{
			ID: id,
			TransactionID: params.TransactionID,
		...
		if err != nil {
			cs.ErrorClear(context.Background(), params.ID, id, 1)
			h.ErrorLog("Cron - Update trx void : " + err.Error())
			fmt.Println(err)
		}
	}
