...
func (cs *CronService) ReversalTrx(params transactiondata.TransactionData) {
	...
	id, err := cs.transactionService.CreateReversalTrx(context.Background(), trxParams)
	if err != nil {
		cs.ErrorClear(context.Background(), params.ID, 0, 0)
		h.ErrorLog("Cron - Save trx reversal : " + err.Error())
		fmt.Println(err)
	}
	...
	if params.DataResponse != "" {
		...
		if params.DataResponse != "E6" {
			...
			if err != nil {
				...
				h.ErrorLog("Cron - Unmarshal response reversal : " + err.Error())
				fmt.Println(err)
			}
		...
		err = cs.transactionService.UpdateReversalTrx(context.Background(), transactions.UpdateSaleParams{
			ID: id,
		...
		if err != nil {
			cs.ErrorClear(context.Background(), params.ID, id, 1)
			h.ErrorLog("Cron - Update trx reversal : " + err.Error())
			fmt.Println(err)
		}
	}
